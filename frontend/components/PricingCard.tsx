...
const PricingCard: React.FC<PricingCardProps> = ({
    ...
    return (
        <HoverCard className={cn("h-full", isPopular && "z-10")}>
            <div className={cn(
                "relative border-2 bg-mambo-panel rounded-2xl p-8 flex flex-col shadow-2xl h-full transition-all duration-300",
                isPopular
                    ? "border-mambo-blue shadow-blue-900/30 hover:scale-[1.02]"
                    : "border-white/10 hover:border-mambo-gold/50 hover:shadow-mambo-gold/10 hover:shadow-2xl hover:scale-[1.02] shadow-blue-900/10"
            )}>
                {isPopular && (
