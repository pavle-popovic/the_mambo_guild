  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className={`relative rounded-lg overflow-hidden border flex flex-col h-full backdrop-blur-sm transition-all duration-300 ${isHighlighted
          ? "bg-gradient-to-r from-amber-900/40 via-yellow-900/30 to-amber-900/40 border-mambo-gold/50 shadow-2xl shadow-mambo-gold/10 z-10 scale-105"
          : "bg-gradient-to-br from-stone-900/60 to-stone-800/40 border-white/10 hover:border-mambo-gold/50 hover:shadow-2xl hover:shadow-mambo-gold/10 z-0 hover:z-10"
          }`}
      >
        {/* Highlighted badge */}
        {isHighlighted && (
        ...
        <div className="p-3">
          ...
          <Link href={tierInfo.ctaLink}>
            <motion.button
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
              className={`w-full py-2 rounded text-xs font-bold transition-all duration-200 ${isHighlighted
