/**
 * Shared body for /salsa-on1-vs-on2 — used by the unprefixed English route
 * and each /[locale]/salsa-on1-vs-on2 variant. All translatable copy is
 * driven by `_content.ts` keyed by locale; structure is locale-agnostic.
 */
...
import type { On1VsOn2Content } from "./_content";
...
/** Render a string with **bold** markers as gold-strong inline emphasis. */
...
/** Render a list-item string where the leading **bold** is the label and
 *  the rest is the body. Used for sec4Bullets, sec5Bullets, sec6Steps. */
...
export default function On1VsOn2Body({
    content,
    locale,
}: {
    content: On1VsOn2Content;
    locale: Locale;
}) {
    ...
    const howToSchema = {
        ...
        "@type": "HowTo",
        "@id": `${PAGE_URL}#howto`,
        name: content.sec6Title,
        description: content.sec6Intro,
        totalTime: "P30D",
                    ...
                    <div className="mx-auto max-w-3xl text-white/85">
                        ...
                        <Section title={content.sec4Title}>
                            ...
                            <ul className="list-disc space-y-2 pl-5 marker:text-mambo-gold">
                                {content.sec4Bullets.map((b, i) => (
                                    <li key={i}><BulletWithBold text={b} /></li>
                                ))}
                            </ul>
                            <p>{content.sec4Outro}</p>
                        </Section>

                        <Section title={content.sec5Title}>
                            <ul className="list-disc space-y-2 pl-5 marker:text-mambo-gold">
                                {content.sec5Bullets.map((b, i) => (
                                    <li key={i}><BulletWithBold text={b} /></li>
                                ))}
                            </ul>
                        </Section>
