    ...
    <div className="min-h-screen bg-mambo-dark">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-8 py-3 lg:py-12 pt-[72px] lg:pt-28">
        ...
        <div className="lg:hidden space-y-3 mb-6">
          ...
          {/* 5. Featured Badges Row */}
          <div>
            <div className="text-[10px] text-gray-500 uppercase tracking-widest font-semibold mb-1.5">Achievements</div>
            {(() => {
              const earnedBadges = user.badges?.filter((b: any) => b.is_earned).slice(0, 5) ?? [];
              if (earnedBadges.length === 0) {
                return (
                  <div className="text-xs text-gray-500 bg-mambo-panel border border-gray-800/40 rounded-lg px-3 py-2 text-center">
              ...
              return (
                <div className="flex gap-2 overflow-x-auto pb-1">
                  {earnedBadges.map((badge: any) => (
                    <div
                      key={badge.id}
                      className="w-12 h-12 shrink-0 rounded-lg border border-mambo-gold/40 bg-mambo-panel flex items-center justify-center shadow-[0_0_6px_rgba(251,191,36,0.15)]"
                      title={badge.name}
                    >
                      {badge.icon_url ? (
                        <img src={badge.icon_url} alt={badge.name} className="w-10 h-10 object-cover rounded" />
                      ) : (
